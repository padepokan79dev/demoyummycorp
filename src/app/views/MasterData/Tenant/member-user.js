...
import { Row, Col, Input, Table, Spin, Space, Button, Checkbox } from 'antd'
...
class MemberUser extends React.Component {
  ...
  async getUserList(search, page, size, sort) {
    this.setState({
      loading: true
    });
    let listTemp = [];
    await FSMServices[this.props.isDetail ? 'getListUserDetailOnTenant' : 'getListUserAddMemberOnTenant'](search, page, size, sort, 'user', this.props.tenantId)
      .then(res => {
  ...
  handleTableChange = (pagination, search, sorter) => {
    ...
    this.setState({
      sort: `${sorter.order ? `${sorter.columnKey},${sorter.order}` : 'tenant_id,user_id,asc'}`,
  ...
  render() {
    const { pagination, selectedRowKeys, userList, loading } = this.state;
